...
j=json.loads((p/'source/Superhero_Female_FullBody.gltf').read_text())
...
out=bytearray((p/'source/Superhero_Female_FullBody.bin').read_bytes())
j['images']=[{'uri':'T_Superhero_Female_Dark_BaseColor.png'},{'uri':'T_Superhero_Female_Normal.png'},{'uri':'T_Eye_Brown.png'}]
...
for an in a['animations']:
 ...
 an['channels']=[c for c in an['channels'] if a['nodes'][c['target']['node']].get('name') in names and (c['target']['path'] != 'translation' or a['nodes'][c['target']['node']].get('name') == 'pelvis')]
 for c in an['channels']:c['target']['node']=names[a['nodes'][c['target']['node']]['name']]
 j['animations'].append(an)
j['buffers']=[{'uri':'boxer_female.bin','byteLength':len(out)}]
(p/'boxer_female.bin').write_bytes(out);(p/'boxer_female.gltf').write_text(json.dumps(j,separators=(',',':')))
