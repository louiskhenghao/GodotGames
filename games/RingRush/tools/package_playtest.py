...
def package():
 web=OUTPUT/'web';mac=OUTPUT/'mac/RingRush.zip'
 notices={
 ...
 temp.replace(mac)
 target=OUTPUT/'RingRush-Web-Playtest.zip'
 ...
 for artifact in [mac,target]:
  ...
  lines.append(digest+'  '+str(artifact.relative_to(OUTPUT)))
  print(artifact.name,round(artifact.stat().st_size/1024**2,1),'MiB',digest)
 (OUTPUT/'SHA256SUMS.txt').write_text('\n'.join(lines)+'\n')
 shutil.copyfile(ROOT/'docs/playtest.md',OUTPUT/'PLAYTEST.md')
