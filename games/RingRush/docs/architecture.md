...
the root `tools/create_game.py` produces an independent starter by copying only `addons/mobile_core/` and a minimal Godot scene. It was imported and launched outside this repository with no `game/` or Ring Rush assets. See `../../../packages/mobile-core/README.md` for API examples and ownership boundaries.
