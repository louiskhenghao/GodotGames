...
const CHARACTERS := [
	...
	{"id":"aegis","name":"AEGIS","style":"THE IRON SENTINEL","price":1800,"hp":190.0,"damage":33.0,"speed":3.9,"tempo":.72,"move":"quake","color":Color("ffa36b"),"passive":"Twin minigun · Armored · Wider shockwaves.","rank":"armor","ranks":2,"range_bonus":.08,"weapon":"minigun"},
	{"id":"ion","name":"ION","style":"THE ARC RUNNER","price":2600,"hp":125.0,"damage":27.0,"speed":5.3,"tempo":.43,"move":"thunder","color":Color("8aefc2"),"passive":"Homing missiles · Agile · Faster recharge.","rank":"chain","ranks":2,"cooldown_bonus":.08,"weapon":"missiles"},
	{"id":"onyx","name":"ONYX","style":"THE NOVA ENGINE","price":3600,"hp":165.0,"damage":38.0,"speed":4.4,"tempo":.57,"move":"meteor","color":Color("c29cff"),"passive":"Flamethrower · Burning cone · Heavy reactor.","rank":"burn","ranks":2,"skill_bonus":.12,"weapon":"flamethrower"}
]
const MOVES := [
	...
	{"id":"pulse","name":"AIR JAB","price":0,"cooldown":5.5,"detail":"Fire three fast energy punches. Available to every fighter.","icon":"target"},
	{"id":"orb","name":"NOVA ORB","price":450,"cooldown":9.0,"detail":"Launch a guided orb that explodes into a crowd.","icon":"nova"}
]
...
static func owned(store: CoreSaveStore, category: String, id: String) -> bool:
	...
	if category == "move":
		if id in ["quake","pulse"]: return true
		for c in CHARACTERS:
...
static func visual(id: String) -> Dictionary:
	return {
		...
		"quake":{"short":"QUAKE","color":Color("ffc04d"),"icon":"quake","hint":"Slam the ground. Stagger a crowd.","tag":"AREA + STAGGER"},
		...
		"pulse":{"short":"AIR JAB","color":Color("65eaff"),"icon":"target","hint":"Three quick energy punches. Aim automatically.","tag":"FREE + RANGED"},
		"orb":{"short":"NOVA ORB","color":Color("e597f3"),"icon":"nova","hint":"A guided blast. Catch a distant crowd.","tag":"HOMING + BLAST"}
	}.get(id,{"short":"QUAKE","color":Color("ffc04d"),"icon":"quake","hint":"Slam the ground.","tag":"AREA"})
