...
for folder in ['web','mac']:(root/'builds'/folder).mkdir(parents=True,exist_ok=True)
...
try:
 ...
 for name in ['Web Playtest','Mac Playtest']:
  subprocess.run([a.godot,'--headless','--path',str(root),'--export-release',name],check=True)
